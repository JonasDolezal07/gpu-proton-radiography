...
\begin{figure}[h!]
  ...
  \caption{Paraxial vs full-orbit radiographs as field amplitude is increased.
  (a--c) Paraxial deflection map. (d--f) Full-orbit \texttt{prad} result.
  Rows: $B_0 = 1, 5, 20$\,T. At $20$\,T the paraxial model misplaces caustics
  by [X]\% and over-predicts peak fluence by [Y]\%.
  }
  \label{fig:paraxial}
\end{figure}
...
Figure~\ref{fig:paraxial} compares a paraxial deflection-map calculation to
\texttt{prad}'s full-orbit result for a z-pinch field at three field amplitudes
(1\,T, 5\,T, 20\,T; $B_0$ scaled uniformly). At low amplitude the two methods agree.
At $20$\,T the paraxial model misplaces the central caustic by [X]\% and
over-predicts the peak fluence by [Y]\%, demonstrating the regime where full-orbit
modelling is required.

\emph{[This section will be completed with computed figures from
\texttt{scripts/fig\_paraxial\_breakdown.py}.]}
...
\begin{figure}[h!]
  ...
  \caption{Synthetic radiographs for (a) monoenergetic 14.7\,MeV,
           (b) Gaussian 14.7\,MeV $\pm 5\%$,
           (c) TNSA exponential $T = 3$\,MeV, $E_\mathrm{cut} = 40$\,MeV.
           Same z-pinch field and geometry. TNSA broadens the caustic due to
           chromatic path-length differences.}
  \label{fig:spectra}
\end{figure}

Figure~\ref{fig:spectra} illustrates how the three supported energy distributions
affect the radiograph of a z-pinch field. The monoenergetic and Gaussian cases
produce sharp caustics; the TNSA exponential spectrum introduces chromatic blurring
proportional to the energy spread. This effect is physically significant in
laser-plasma experiments where source energy spread is large.

Parameter sweeps make it straightforward to study how the radiograph evolves with
a continuously varying parameter. Figure~\ref{fig:sweep} shows a 10-point sweep
over proton energy from 5\,MeV to 50\,MeV through the same field, illustrating
the transition from non-relativistic ($\gamma \approx 1.005$ at 5\,MeV) to
mildly relativistic ($\gamma \approx 1.053$ at 50\,MeV) dynamics.

\begin{figure}[h!]
  \centering
  % PLACEHOLDER — generate with scripts/fig_energy_sweep.py
  \includegraphics[width=\linewidth]{figures/energy_sweep}
  \caption{Energy sweep: radiographs at $E = 5, 10, 20, 30, 40, 50$\,MeV
           through the same z-pinch field. Peak deflection decreases as $1/p$
           with increasing momentum; the caustic sharpens.}
  \label{fig:sweep}
\end{figure}
